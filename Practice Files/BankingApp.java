...
class LoanOverdueException extends Exception{
    LoanOverdueException(String message){
        super("Loan is overdue by more than 60 days. Immediate payment required.");
    }
}

class NegativeLoanRepaymentAmountException extends Exception{
    NegativeLoanRepaymentAmountException(String message){
        super(message);
    }
}

class InsufficientFundsForLoanRepaymentException extends Exception{
    InsufficientFundsForLoanRepaymentException(String message){
        super(message);
    }
}

public class BankingApp {
    static double[] processingTransaction(double b,double o,double m,int n,double w) throws LoanOverdueException, NegativeLoanRepaymentAmountException,InsufficientFundsForLoanRepaymentException{
        if(n>60){
    ...
    public static void main(String args[]){
       ...
       try{ System.out.println("Enter the initial bank balance:");    
        double b=sc.nextDouble();
        System.out.println("Outstanding loan amount: ");
        double o=sc.nextDouble();
        System.out.println("Minimum loan repayment amount: ");
        double m=sc.nextDouble();
        System.out.println("Number of days the loan is overdue: ");
        int n=sc.nextInt();
        System.out.println("Amount to withdraw from account: ");
        double w=sc.nextDouble();

        double[] result=processingTransaction(b,o,m,n,w);
